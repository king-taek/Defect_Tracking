...
## 1-3. REVIEW-03 결정 반영 (QUESTIONS-03 답변)

판정은 **승인 + 정정 3건**. REVIEW-02 의 세 항목이 틀렸고 제 지적이 맞다는 확인입니다.

| 질의 | 결정 | 반영 |
|---|---|---|
| Q18 clamp 포화 | **A**: V 포화 후 채도 하강. 조용한 실패 금지(게이트) | 목표 미달 시 `ValueError`. 임의 색 324조합에서 미달·예외 0건 |
| Q19 목표값 | **B**: `ACCENT_CLAMP_TARGET = 5.4`, 합격선은 5.0 유지 | 생성 목표와 판정 기준 분리. 목표 > 합격선은 게이트 |
| Q20 표 vs 알고리즘 | 제안 승인: 표 = 프리셋 상수(우선), 규칙 = 임의 색 안전망 | `ACCENT_PRESETS` 를 `{light, dark}` 로, `None` 은 규칙 위임 |
| Q21 경계선 3.0 | **A**: 하위 기준 대상을 포커스 링·선택 링으로 한정. A17 경계선 항목 **철회** | xfail 4건을 남기지 않고 게이트 대상에서 제거. 대신 hover 배경 변화 게이트 추가 |
| Q22 선택 링 | **A**: 이중선(바깥 accentFill / 안쪽 onAccent). B는 수치상 불충분(약 1.7) | `SELECTION_RING` 상수 + 램프 양 끝 회귀. 램프 상한 1.0 유지 |
| hover·pressed 반전 | 승인 + 게이트 추가 | `onAccent × accentHover/accentPressed` 를 기본 쌍 목록에 추가 |
| `text` clamp | 승인 + 정정: 페이지 면 기준 | card·layer·win **세 면 최악** 기준으로 clamp. 카드만 보면 무작위 150색 중 143건이 layer/win 에서 미달했다 |

**토큰 레이어는 이 왕복으로 닫혔습니다.** 다음 질의는 레이아웃 실측(단계 2 판독대 4열,
단계 6 래스터 LOD)에서 나올 예정입니다.
...
- **accent 생성(A18/A19)**: 목표 5.4 로 생성하고 5.0 으로 판정. clamp 는 V 포화 시 채도를 낮춰
  이어가고, 실패하면 예외로 드러낸다(조용한 미달 종료 금지 - 게이트).
- **하위 기준 대상(A21)**: 포커스 링과 히트맵 선택 링만. 카드·컨트롤 경계선은 게이트·하위 기준
  모두에서 제외하되, **hover·disabled 를 경계선 단독으로 표현하지 않는다**(게이트).
  hover 는 배경 변화를 동반하고 disabled 는 채움이 바탕 쪽으로 수축한다.
- **히트맵 선택 링(A22)**: die 사각 안쪽 2px 이중선. 바깥 1px `accentFill`, 안쪽 1px `onAccent`.
  러버밴드도 같은 이중선 + `Qt.DashLine`. 램프 상한은 1.0 유지.
- **상위 근거**: `02-design-rules.md` §0·§1.3·§1.4 와 `03-screens.md` §3 선택 링은
  REVIEW-02·REVIEW-03 이 대체한다(원본 문서는 수정하지 않는다).

## 5. 미해결 (착수 전 답변 필요)

| 항목 | 상태 | 막는 범위 |
|---|---|---|
| **Q1 라이선스** | 2026-08-23 사용자 확인: **진행** | 해소 |

Q1(qfluentwidgets GPLv3 - 비상업 한정)은 세 차례 제기했고 사용자가 전권 위임과 함께 진행을
지시했으므로 사업 주체의 진행 결정으로 기록합니다. 이 결정이 번복되면 되돌릴 지점은 **커밋 ③
(의존성 고정, `3e98452`)** 하나입니다. 그래서 의존성 고정을 별도 커밋으로 유지했습니다.

QUESTIONS-02·QUESTIONS-03 은 REVIEW-02·REVIEW-03 으로 모두 해소되어 strict xfail 이 0건입니다.
토큰 레이어가 닫혔고 라이선스도 확인되어 남은 차단 요소는 없습니다.
