...
def test_scan_detects_all_sources(lot):
    ...
    assert set(idx.layer_canonicals()) == {"LYA4", "LYB4", "LYA3", "LYB3"}
...
def test_matching_finds_cross_layer(lot):
    ...
    base = [r for r in idx.records_for_layer("LYA4") if r.ok]
    compare = ["LYB4", "LYA3", "LYB3"]
...
def test_export_and_source_untouched(lot, tmp_path):
    ...
    rbl = idx.records_by_layer()
    base = [r for r in idx.records_for_layer("LYA4") if r.ok]
    results = matcher.match_all(base, ["LYB4", "LYA3"], rbl, config.DEFAULT_TOLERANCE)
...
def test_export_into_source_blocked(lot, tmp_path):
    ...
    rbl = idx.records_by_layer()
    base = [r for r in idx.records_for_layer("LYA4") if r.ok]
    results = matcher.match_all(base, ["LYB4"], rbl, config.DEFAULT_TOLERANCE)
    cache = ThumbnailCache(tmp_path / "ws" / "cache")
...
def test_rereview_layers_kept_distinct(tmp_path):
    ...
    lot = tmp_path / "204. DEVA"
    # 같은 canonical(LYA4) 의 일반 + 재리뷰 폴더, 그리고 충돌 없는 LYB4
    (lot / "1. LYA4" / "W1").mkdir(parents=True)
    (lot / "2. LYA4_재리뷰" / "W1").mkdir(parents=True)
    (lot / "3. LYB4" / "W1").mkdir(parents=True)
    # 각 wafer 폴더에 좌표가 파일명에 있는 Camtek 이미지 1장씩
    for folder in ("1. LYA4", "2. LYA4_재리뷰", "3. LYB4"):
        img = lot / folder / "W1" / "R_DEVA_LIVE_X_PKG_X_W1_4_5_1000.000000_2000.000000_Defect.jpg"
        img.write_bytes(b"\xff\xd8\xff\xd9")  # 최소 JPEG 바이트(좌표는 파일명에서 추출)

    idx = scanner.scan_lot(lot)
    names = idx.layer_canonicals()
    # 충돌난 LYA4 는 일반/재리뷰로 구분, 충돌 없는 LYB4 는 그대로
    assert "LYA4" in names
    assert "LYA4_재리뷰" in names
    assert "LYB4" in names
    # records 도 섞이지 않고 각 display 로 분리
    rbl = idx.records_by_layer()
    assert rbl["LYA4"] and all(r.layer == "LYA4" for r in rbl["LYA4"])
    assert rbl["LYA4_재리뷰"] and all(r.layer == "LYA4_재리뷰" for r in rbl["LYA4_재리뷰"])
