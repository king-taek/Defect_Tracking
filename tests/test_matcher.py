...
def test_large_systematic_offset_auto_corrected():
    ...
    bases = [
        ...
        _rec("RDL4", "W1", 3, 3, 1000.0, 0.0),
        _rec("RDL4", "W1", 5, 5, 1000.0, 0.0),
        _rec("RDL4", "W1", 1, 5, 1000.0, 0.0),
    ]
    cmps = [
        _rec("PI4", "W1", 1, 1, 1150.0, 0.0, name="a.jpg"),
        _rec("PI4", "W1", 3, 3, 1150.0, 0.0, name="b.jpg"),
        _rec("PI4", "W1", 5, 5, 1150.0, 0.0, name="c.jpg"),
        _rec("PI4", "W1", 1, 5, 1150.0, 0.0, name="d.jpg"),
    ]
    matches, offsets = match_all_with_offsets(bases, ["PI4"], {"PI4": cmps}, tolerance=100.0)
    assert offsets["PI4"].count == 4 and round(offsets["PI4"].dx) == -150
    assert all(m.for_layer("PI4").is_match for m in matches)
...
def test_inconsistent_offsets_not_applied():
    ...
    cmps = [
        _rec("PI4", "W1", 1, 1, 1150.0, 0.0),
        _rec("PI4", "W1", 3, 3, 1600.0, 0.0),
        _rec("PI4", "W1", 5, 5, 800.0, 0.0),
    ]
    matches, offsets = match_all_with_offsets(bases, ["PI4"], {"PI4": cmps}, tolerance=100.0)
    assert offsets["PI4"].count == 0
    assert not any(m.for_layer("PI4").is_match for m in matches)
...
def test_normalize_layer_order_prefix_and_rereview():
    ...
    assert layout.normalize_layer("3. RDL4_재재리뷰") == ("RDL4", True)


def test_re_review_level():
    assert layout.re_review_level("1. RDL4") == 0
    assert layout.re_review_level("1. RDL4_재리뷰") == 1
    assert layout.re_review_level("2. RDL4_재재리뷰") == 2
    assert layout.re_review_level("3. RDL4_재재재리뷰") == 3
    assert layout.re_review_level("RDL4_rereview") == 1
    assert layout.re_review_level("RDL4_re-re-review") == 2
