...
생성되는 layer:
  1. RDL4_재리뷰  -> Camtek 파일명형(좌표가 파일명에 포함)
  2. PI4_재리뷰   -> Camtek INI형(ColorImageGrabingInfo.ini section 으로 좌표 산출)
  3. RDL3        -> KLA형(.jpg + .001 info + .pass)
  4. PI3         -> Camtek 파일명형
...
# layer 정의: (폴더명, 방식)
LAYERS = [
    ("1. RDL4_재리뷰", "camtek_name"),
    ("2. PI4_재리뷰", "camtek_ini"),
    ("3. RDL3", "kla"),
    ("4. PI3", "camtek_name"),
]
...
def _gen_kla(folder: Path, layer_tag: str, wafer: str) -> None:
    ...
    (folder / f"{uuid}_TB500_{wafer}.001").write_text(
        "\n".join(info_lines), encoding="utf-8"
    )
    (folder / f"{uuid}_TB500_{wafer}.pass").write_text("ignore me", encoding="utf-8")
